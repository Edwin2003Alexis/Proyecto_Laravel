...
<x-app-layout>
    <div class="container mx-auto mt-12 p-8 bg-white rounded-lg shadow-lg">
        <h1 class="text-4xl font-bold text-center text-gray-800" style="color: #198754">Productos Disponibles</h1>

        <!-- Mostrar mensaje de éxito -->
        @if (session('success'))
            <div id="success-message" class="bg-green-600 text-white p-4 rounded-lg mb-4">
                {{ session('success') }}
            </div>
        @endif
            ...
            @foreach($products as $product)
                <div class="border rounded-lg p-4">
                    ...
                    <form action="{{ route('comprador.cart.add', $product->id) }}" method="POST">
                        @csrf
                        <button type="submit" class="mt-2 bg-green-500 text-white rounded-lg py-2 px-4">Comprar</button>
                    </form>
                </div>
            @endforeach
        </div>
